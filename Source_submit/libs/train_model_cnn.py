...
# Model parameters
IMG_SIZE = 299  # ! Tensor input size
...
class CustomXceptionModel(nn.Module):
    def __init__(self, num_classes, pretrained=True):  # Added pretrained flag
        super(CustomXceptionModel, self).__init__()

        # Load the pre-trained Xception model using timm
        # Note: 'xception' is the model name in timm
        ...
        # In timm's Xception, the classifier layer is typically named 'fc'
        num_ftrs = self.base_model.fc.in_features

        # Remove the original classifier by replacing it with an Identity layer
        # This makes the base_model output features before the final classification
        self.base_model.fc = nn.Identity()

        # Add new custom layers
        self.flatten = nn.Flatten()
        self.dropout1 = nn.Dropout(p=0.3)
        # Use the dynamically obtained 'num_ftrs' for the input size
        self.fc1 = nn.Linear(num_ftrs, 128)
        self.relu = nn.ReLU()  # Define ReLU explicitly or use F.relu in forward
        self.dropout2 = nn.Dropout(p=0.25)
        # Output layer for the number of classes
        self.fc2 = nn.Linear(128, num_classes)

    def forward(self, x):
        # Pass through the base model (outputs features)
        x = self.base_model(x)
        # Flatten might be redundant if base_model already outputs (batch, features)
        # but usually safe to keep.
        x = self.flatten(x)
        x = self.dropout1(x)
        x = self.fc1(x)
        x = self.relu(x)      # Apply ReLU activation (or use F.relu(x))
        x = self.dropout2(x)
        x = self.fc2(x)         # Output layer
        return x


def create_model(num_classes):  # Added use_pretrained flag
    """Create and configure the Custom Xception model"""
    print(
        f"Creating Custom Xception model with num_classes={num_classes} and pretrained={True}")
    # Use the custom architecture defined earlier
    model = CustomXceptionModel(
        num_classes=num_classes,
        pretrained=True
    )

    # Initialize image processor - Placeholder, usually defined elsewhere with transforms
    # Typically you'd define torchvision.transforms outside this function
    # and apply them in your Dataset/DataLoader.
    image_processor = None
    print("Note: Image processor/transforms should be defined separately.")

    # Unfreeze all layers for fine-tuning (this is the default state anyway for a new model,
    # but explicit here if you were adapting from code that froze layers)
    # If pretrained=False, all requires_grad are True by default.
    # If pretrained=True, they are also True by default unless frozen earlier.
    # This loop ensures everything is trainable.
    print("Ensuring all model parameters require gradients (for fine-tuning).")
...
# Add this FocalLoss class above your main code (e.g., after imports)
...
def main():
    ...
    # ! Training setup, Update the criterion in your main function
    # criterion = nn.CrossEntropyLoss()
    criterion = FocalLoss()
    print("Use Focal loss to training.")
